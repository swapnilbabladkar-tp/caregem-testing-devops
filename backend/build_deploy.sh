...
run_build_deploy(){
    ...
    if echo "${changed_service_list[@]}" | grep -q "layers"; then
        ...
        list_folders=$(echo "$folders" | sed 's/backend//g' | sed 's/\.//g' | sed 's/\///g')

        for service_name in ${list_folders[@]}; do

            # Build file
            buildfile=./backend/${service_name}/${service_name}.yaml
            sam_build $service_name $buildfile

            # Deploy file
            deployfile=./backend/${service_name}/${service_name}-config.toml
